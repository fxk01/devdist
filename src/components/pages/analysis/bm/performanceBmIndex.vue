...
<template>
    <div class="container">
        ...
        <el-row :gutter="20">
            ...
            <el-col :span="12">
                <div class="chart-box">
                    <v-chart style="width:100%;" :options="personOptions" autoresize />
                    <v-echarts-no-data :isShow="isPerson"></v-echarts-no-data>
                </div>
            </el-col>
        </el-row>
    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                isPerson: 0,    //是否显示暂无数据，0为显示，非0为不显示
                personOptions: {
                    title: {
                        text: '個人業績佔比'
                    },
        ...
        methods: {
            ...
            //获取业绩完成情况
            getFinishOptions() {
                this.$post('/api/WorkSpaceApi/GetAchieveCompleteByDir', { DateRangeType: this.timeRange })
                .then((res) => {
                    this.isFinish = res.Info.length;
                    let coordinateArr = [], expectArr = [], finishArr = [], iconArr = [];
                    res.Info.forEach((item, index) => {
                        coordinateArr.push(item.Stf_Name);
            ...
            //获取产品业绩占比
            getProductOptions() {
                this.$post('/api/AnalysisApi/GetProductAchieveByDir', { DateRangeType: this.timeRange })
                .then((res) => {
            ...
            //获取个人业绩占比
            getPersonOptions() {
                this.$post('/api/AnalysisApi/GetAgentAchieveByDir', { DateRangeType: this.timeRange })
                .then((res) => {
                    this.isPerson = res.Info.length;
                    let legendArr = [], proportionArr = [];
                    res.Info.forEach(item => {
                        legendArr.push(item.Stf_Name);
                        proportionArr.push({
                            name: item.Stf_Name,
                            value: item.AgentMoney
                        });
                    });
                    this.personOptions.legend.data = legendArr;
                    this.personOptions.series[0].data = proportionArr;
                });
            },
            //获取数据列表
            getData() {
                ...
                this.$post('/api/AnalysisApi/GetPageAchieveDataByDir', params)
                .then((res) => {
