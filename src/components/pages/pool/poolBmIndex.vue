            ...
            <el-table :data="tableData" empty-text="暫無數據" @selection-change="tableSelectionChange">
                ...
                <el-table-column label="操作" width="300">
                    <template slot-scope="scope">
                        ...
                        <el-button v-if="processState === 0" type="primary" size="mini" @click="showAllocationDialog('分配客戶', [scope.row])">分配客戶</el-button>
                        <el-button v-if="processState === 0" type="warning" size="mini" @click="setInvalid(scope.row)">設為無效</el-button>
                    </template>
                </el-table-column>
            </el-table>
    ...
    export default {
        ...
        mounted() {
            Promise.all(
                [
                    ...
                    this.$store.dispatch('getTypeData', '主管未處理')
                ]
            ).then(() => {
                ...
                if(this.$route.query.text === sessionStorage.getItem('fc_userid')) {
                    ...
                    this.status = this.statusList.filter(item => item.BO_Value === '已分配主管');
                    this.dateRadio = '全部';
                    this.datePicker = ['', ''];
                }
                this.getData();
        ...
        methods: {
            ...
            changeProcessState(v) {
                this.resetForm(v);
                if(v === 1) {
                    this.$store.dispatch('getTypeData', '主管已處理').then(res => {
                        this.statusList = this.$store.state.poolStatusList;
                        // this.status = this.$store.state.poolStatusList;
                        this.queryData();
                    });
                } else {
                    this.$store.dispatch('getTypeData', '主管未處理').then(res => {
                        this.statusList = this.$store.state.poolStatusList;
                        // this.status = this.$store.state.poolStatusList;
                        this.queryData();
                    });
            ...
            getData() {
                ...
                this.$post('/api/ClueApi/GetPagePoolByDir', params)
                .then((res) => {
